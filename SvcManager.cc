...
namespace MatchIo {
SvcManager::SvcManager(const TCHAR* svcName, const TCHAR* fileName) : schSCManager{ 0 }, schService{ 0 }
{
    TCHAR szUnquotedPath[MAX_PATH];
    StringCchCopy(szSvcName, SVCNAME_LEN, svcName);
    if (!GetCurrentDirectory(MAX_PATH, szUnquotedPath)) {
        printf("Cannot get current directory (%d)\n", GetLastError());
        throw std::exception("Cannot get current directory");
    }
    // kernel driver path cannot have quotes !!!
...
VOID SvcManager::SvcInstall()
{
    ...
    if (schService)
    {
        printf("Service already exist (%d)\n", GetLastError());
...
/*
VOID __stdcall SvcManager::DoQuerySvc()
{
...
*/
VOID __stdcall SvcManager::DoDisableSvc()
{
    // Get a handle to the SCM database. 
...
VOID __stdcall SvcManager::DoEnableSvc()
{
    // Get a handle to the SCM database. 
...
VOID __stdcall SvcManager::DoDeleteSvc()
{
    // Get a handle to the SCM database. 
...
VOID __stdcall SvcManager::DoStartSvc()
{
...
VOID __stdcall SvcManager::DoStopSvc()
{
...
BOOL __stdcall SvcManager::StopDependentServices()
{
    DWORD i;
    DWORD dwBytesNeeded;
    DWORD dwCount;
